...
import * as sushiV2EthFactoryAbi from "../abi/sushiswap_v2_ethereum_factory"
import * as sushiV2EthPoolAbi from "../abi/sushiswap_v2_ethereum_pool"
...
processor.run(
  new TypeormDatabase({
    ...
    stateSchema: "eth_processor_sushiswap_v2",
  }),
  async (ctx) => {
    ...
    for (let block of ctx.blocks) {
      for (let log of block.logs) {
        ...
        if (
          log.topics[0] == sushiV2EthFactoryAbi.events.PairCreated.topic &&
          ETH_ADDRESS == log.address
        ) {
        ...
        if (
          log.topics[0] == sushiV2EthPoolAbi.events.Swap.topic
          &&
          factoryPools.has(log.address)
        ) {
...
function decodePoolLog(log: Log) {
  ...
  event = sushiV2EthFactoryAbi.events.PairCreated.decode(log);
  return {
    pool: event.pair,
    token0: event.token0,
    token1: event.token1,
...
function decodeSwapLog(log: Log) {
  let event;
  event = sushiV2EthPoolAbi.events.Swap.decode(log);
  return {
