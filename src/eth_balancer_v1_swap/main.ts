...
processor.run(
  new TypeormDatabase({
    ...
    stateSchema: "eth_processor_balancerV1",
  }),
  async (ctx) => {
    ...
    for (let block of ctx.blocks) {
      for (let log of block.logs) {
        ...
        if (
          log.topics[0] == balancerV1EthFactoryAbi.events.LOG_NEW_POOL.topic &&
          ETH_ADDRESS == log.address
        ) {
        ...
        // } else
        if (
          log.topics[0] == balancerV1EthPoolAbi.events.LOG_SWAP.topic 
          // &&
          // factoryPools.has(log.address)
        ) {
...
function decodePoolLog(log: Log) {
  ...
  event = balancerV1EthFactoryAbi.events.LOG_NEW_POOL.decode(log);
  return {
    pool: event.pool,
  };
}
...
function decodeSwapLog(log: Log) {
  let event;
  event = balancerV1EthPoolAbi.events.LOG_SWAP.decode(log);
  return {
    token0 : event.tokenIn,
    token1: event.tokenOut,
    amount0: event.tokenAmountIn,
    amount1: event.tokenAmountOut,
  };
}
